...
== Supported renderers
This fork currently supports two different renderers. Besides the <tt>:bootstrap</tt> renderer, there is also the <tt>:bootstrap_split</tt> renderer that splits the navigation items into two lists.
To use it, simply select it instead:
 render_navigation :expand_all => true, :renderer => :bootstrap_split
